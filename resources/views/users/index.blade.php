...
<div class="p-4 xl:ml-64">
    <div class="py-4 rounded-lg dark:border-gray-700 mt-14">
        <div class="flex flex-col xl:flex-row justify-start xl:justify-between items-start xl:items-center mb-5">
            <div class="mb-2">
                <h2 class="text-gray-100 text-3xl font-medium tracking-wider">Project Management</h2>
            </div>
        </div>
        @if ($count > 0)
            <div class="grid grid-cols-1 xl:grid-cols-3 gap-4 mb-4">
                @foreach ($list as $project)
                    <a href="{{ route('auth.organization.view', $project->uuid) }}" class="min-h-[6rem] block rounded bg-gray-800 dark:bg-gray-800 p-4 hover:cursor-pointer border border-gray-800 hover:border-gray-400">
        ...
        @else
            <div class="flex items-center justify-center h-[30rem] mb-4 rounded dark:bg-gray-800">
                <p class="text-2xl text-gray-400 dark:text-gray-500">
                    No available project management
                </p>
            </div>
        @endif
    </div>
